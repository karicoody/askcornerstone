...
export default function Page() {
  ...
  async function ask(e: React.FormEvent) {
    ...
    if (!q.trim()) return;
    const yourQ = q.trim();
    setLog(l=>[...l, {who:"you", text: yourQ}]); setQ("");
    const r = await fetch("/api/chat", {method:"POST", headers:{'Content-Type':'application/json'}, body: JSON.stringify({message: yourQ})});
  ...
  return (
    <div style={{maxWidth:760, margin:"24px auto"}}>
            ...
            {m.sources?.length ? (
              <div style={{fontSize:12, background:"#E4D9CE", padding:8, borderRadius:8, marginTop:6}}>
                ...
                <ul style={{margin:"4px 0 0 16px"}}>
                  {m.sources.slice(0,6).map((s,idx)=><li key={idx}>{s.title}{s.page?` (p.${s.page})`:""}</li>)}
                </ul>
              </div>
            ):null}
          </div>
      ...
      <form onSubmit={ask} style={{display:"flex", gap:8, marginTop:8}}>
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Ask about minerals, nervous system, bloodwork, etc."
      ...
      <p><a href="/admin" style={{fontSize:14}}>Admin: upload files</a></p>
    </div>
  );
}
